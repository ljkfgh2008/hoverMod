	function hoverMod(itemClass,introClass){
		var _this=this;
		//获得所有hoverItem
		...
		for (var i = 0; i < sItemArr.length; i++) {
			...
			var Intro=_this.getByClass(thisItem,introClass)[0];
			//获取宽度高度
 			thisItem.size={
			...
			Intro.style.width=thisItem.size.width+"px";
			Intro.style.height=thisItem.size.height+"px";
			Intro.style.top=thisItem.size.height+"px";
			Intro.style.left="0px";
			Intro.style.zIndex="99";
			...
			eventRegister(thisItem,Intro);
		}
	}

	hoverMod.prototype.set=function(json){
		for( i in json){
			this[i]=json[i];
		}
	}
	hoverMod.prototype.trigger=function(item,dir,ev){
		try{
	...
	hoverMod.prototype.hoverIn=function(ev,item,intro){
		var e=ev||event;
	...
	hoverMod.prototype.hoverOut=function(ev,item,intro){
		var e=ev||event;
		var opacity=100;
	 ...
	 hoverMod.prototype.startMove=function(obj,json,fn){
		var nowAttr;
		var speed;
	 ...
	 hoverMod.prototype.myAddEvent=function(obj,evName,fn){
	...
	hoverMod.prototype.getByClass=function(oParent,className){
		var aResult=[];
	...
	hoverMod.prototype.getStyle=function(obj,attr){
		var value;
	...
	hoverMod.prototype.getScroll=function(){
		var scroll={
	...
	hoverMod.prototype.getPos=function(obj) {
	...
	hoverMod.prototype.getDir=function(e,item){
		var e=e||event;
